...
process.env.BACKEND_CLOUD_URL : process.env.BACKEND_LOCAL_URL;
...
app.get('/posts', async (req, res) => {
    try {
        ...
        // Map the posts to include the populated user data
        const formattedPosts = posts.map(post => {
            const populatedUser = post.author._id;
            return {
                ...post.toObject(),
                author: {
...
app.post('/uploadProfilePic', authenticateToken, multer.single('profilePic'), async (req, res) => {
    if (!req.file) {
    ...
    try {
        ...
        const profilePicUrl = `${BACKEND_URL}/uploads/${req.file.filename}`;    
        user.profilePic = profilePicUrl;
        await user.save();

        res.status(200).json({ message: 'Profile pic uploaded successfully.', profilePicUrl });
    } catch (error) {
        return res.status(500).json({ message: 'Error uploading the Profile picture.', error: error.message });
    }
});
